...
def poly3(x, a, b, c, d):
  xi = np.arange(65, 150)
  poly_norm = sum(poly3_nonorm(xi, a, b, c, d))
  
  return poly3_nonorm(x, a, b, c, d) + (-poly_norm + 1) / 85
...
def plot(MC, uncert, bin_centers, f, popt, savepath, ratio_ylim, bonly=False, band=False):
  ...
  axs[0].plot(x, f(x, *popt), label="Bern4 fit")
  ...
  if band:
    ...
    axs[0].fill_betweenx(axs[0].get_ylim(), 80, 100, color="grey", alpha=0.5)
    axs[1].fill_betweenx(axs[1].get_ylim(), 80, 100, color="grey", alpha=0.5)
...
def plotSpB(MC, uncert, bin_centers, f, popt, savepath, ratio_ylim, bonly=False):
  ...
  axs[0].errorbar(bin_centers, MC, yerr=uncert, fmt="ko")
  axs[0].plot(x, f(x, *popt), label="DCB + Bern4 fit")
  axs[0].plot(x, f(x, *bkg_popt), label="Bern4 component fit")
...
bkg_f = lambda x, N, a, b, c, d: N*poly3(x, a, b, c, d)
spb_f = lambda x, N, a, b, c, d, mean, sigma, beta1, m1, beta2, m2, frac: N*(frac*dcb(x, mean, sigma, beta1, m1, beta2, m2) + (1-frac)*poly3(x, a, b, c, d))
...
p0 = [1, *np.random.uniform(-10, 10, 4)]
bounds = [[0.5, 2],
          [-10, 10],
          [-10, 10],
          [-10, 10],
...
print(df_inv.weight.sum())
print(f"Inverted veto falling bkg: {df_inv.weight.sum()*(1-spb_popt[-1])}")
...
plot(MC_minus_DCB, uncert, bin_centers, spb_f, bkg_popt, "inverted_fit_spb_subtract.png", (0, 2), band=True)
...
MC, uncert, bin_centers = histogram(df)
plot(MC, uncert, bin_centers, bkg_f, bkg_popt, "veto_fit_bonly.png", (0, 2))
...
p0[5:-1] = spb_popt[5:-1]
bounds[0, 5:-1] = p0[5:-1] - 0.000001
bounds[1, 5:-1] = p0[5:-1] + 0.000001

res = spo.curve_fit(spb_f, bin_centers, MC, p0, uncert, bounds=bounds)
spb_popt = res[0]
print(list(spb_popt))

plotSpB(MC, uncert, bin_centers, spb_f, spb_popt, "veto_fit_spb_inverted_dcb.png", (0, 2))

print(df.weight.sum())
print(f"Inverted veto falling bkg: {df.weight.sum()*(1-spb_popt[-1])}")
...
plotSpB(MC, uncert, bin_centers, spb_f, spb_popt, "veto_fit_spb.png", (0, 2))